...
<div class="bg-[#14141a] dark:bg-[#111111] text-gray-900 dark:text-white min-h-screen flex flex-col justify-between relative overflow-hidden rounded-t-[3rem]">
    ...
    <!-- Footer Bawah -->
    <footer class="bg-[#14141a] dark:bg-[#111111] border-t border-gray-600">
        <div class="max-w-7xl mx-auto px-6 py-6">
           <!-- Container utama footer: ubah menjadi flex-col agar selalu vertikal -->
<div class="flex flex-col gap-6">
    ...
    <!-- Kanan: Links & Copyright (sekarang di baris kedua) -->
    <div class="flex flex-col md:flex-row items-center justify-center gap-6 text-sm text-gray-600 dark:text-gray-400 mt-3">
        <nav class="flex flex-wrap justify-center gap-6">
            ...
            <!-- Cookie Settings Link (dengan ID) -->
            <a href="#" id="cookie-settings" class="hover:text-white transition">Cookie settings</a>
...
<!-- =====================================================
     COOKIE SETTINGS MODAL (BOTTOM SHEET)
====================================================== -->
<div id="cookie-modal" class="fixed inset-0 z-[999] hidden items-end justify-center">
    <div class="w-full max-w-2xl bg-white dark:bg-[#111111] border border-gray-200 dark:border-gray-700 rounded-2xl p-6 md:p-8 shadow-2xl slide-up mb-4">
        <div class="flex flex-col md:flex-row items-center justify-between gap-6">
            <!-- Teks Kiri -->
            <div class="flex-1 text-center md:text-left">
                <p class="text-gray-900 dark:text-white text-lg font-medium leading-relaxed">
                    We use cookies and similar technologies to see how this site is used. 
                    <a href="<?= base_url('privacy-policy') ?>" class="text-gray-900 dark:text-white underline underline-offset-4 hover:text-[#ff2500] transition">Privacy Policy</a>
                </p>
            </div>
            
            <!-- Tombol Kanan -->
            <div class="flex items-center gap-3">
                <!-- Tombol Decline: Latar transparan di light, border abu-abu -->
                <button id="cookie-decline" class="px-6 py-3 rounded-full border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white font-semibold hover:bg-gray-100 dark:hover:bg-white/10 transition">
                    Decline
                </button>
                <!-- Tombol Accept: Merah di kedua mode -->
                <button id="cookie-accept" class="px-8 py-3 rounded-full bg-[#ff2500] text-white font-semibold hover:bg-[#e02000] transition">
                    Accept
                </button>
            </div>
        </div>
...
<script>
    ...
    // Tutup modal dan pilih Accept
    if (acceptBtn && cookieModal) {
        acceptBtn.addEventListener('click', function() {
            cookieModal.classList.add('hidden');
            cookieModal.classList.remove('flex');
            // Logic simpan preferensi Accept
            localStorage.setItem('cookie_consent', 'accepted');
        });
    }

    // Tutup modal dan pilih Decline
    if (declineBtn && cookieModal) {
        declineBtn.addEventListener('click', function() {
            cookieModal.classList.add('hidden');
            cookieModal.classList.remove('flex');
            // Logic simpan preferensi Decline
            localStorage.setItem('cookie_consent', 'declined');
        });
    }

    // Klik di luar area modal untuk menutup (opsional)
    if (cookieModal) {
